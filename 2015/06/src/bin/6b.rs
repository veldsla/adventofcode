...
impl Grid {
    ...
    fn total_brightness(&self) -> i64 {
        ...
        for r in 0..1000 {
            for l in 0..1000 {
                sum += self.lights[r][l].brightness();
            }
        }
        sum
    }
...
fn main() {
    ...
    println!("The total brightness is {}", g.total_brightness());
}

//for some reason cargo test fails. Program generates correct answer....
#[test]
fn test_grid() {
    let mut g = Grid::new();
    assert_eq!(g.total_brightness(), 0);
    g.change(499,499,500,500, ChangeLight::TurnOn);
    assert_eq!(g.total_brightness(), 4);
    g.change(0,0,0,999, ChangeLight::Toggle);
    assert_eq!(g.total_brightness(), 2004);
    g.change(0,600,999,600, ChangeLight::TurnOff);
    assert_eq!(g.total_brightness(), 2003);
}
